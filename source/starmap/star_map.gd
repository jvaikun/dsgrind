...
func set_focus(value):
	...
	tween_cam.tween_property($MapCam, "position", sector_list[focus_index].position, 1.0)
	tween_cam.play()
...
func gen_map():
	for i in FACTION_COUNT:
		...
		$Menu/SideBar/Body/TabBar/Factions.add_item({
	...
	for j in INTEL_COUNT:
		...
		$Menu/SideBar/Body/TabBar/Intel.add_item({
...
func gen_missions():
	...
	for i in MISSION_COUNT:
		...
		$Menu/SideBar/Body/TabBar/Missions.add_item({
...
func _on_Missions_item_selected(item):
	...
	var tween_cam = create_tween()
	tween_cam.tween_property($MapCam, "position", item.object.position - Vector2(256, 0), distance / SCROLL_SPEED)
	tween_cam.play()
